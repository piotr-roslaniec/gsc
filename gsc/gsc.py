...
class DistroRetrievalError(Exception):
    def __init__(self, *args):
        super().__init__(
            (
                "Could not automatically detect the OS distro of the supplied Docker "
...
def build_docker_image(docker_api, build_path, image_name, dockerfile, **kwargs):
    build_path = str(
        build_path
    )  # Docker API doesn't understand PathLib's PosixPath type
    stream = docker_api.build(
        path=build_path, tag=image_name, dockerfile=dockerfile, decode=True, **kwargs
    )
    for chunk in stream:
        if "stream" in chunk:
...
def extract_environment_from_image_config(config):
    ...
    for env_var in env_list:
        ...
        if "\n" in env_var:
            ...
            print(
                f'Skipping environment variable `{env_var.split("=", maxsplit=1)[0]}`: '
...
def extract_define_args(args):
    ...
    for item in args.define:
        if "=" in item:
            ...
            print(
                f"Invalid value for argument `{item}`, expected `--define {item}=<value>`"
            )
            sys.exit(1)
...
def merge_manifests_in_order(
    manifest1, manifest2, manifest1_name, manifest2_name, path=[]
):
    for key in manifest2:
        if key in manifest1:
            if isinstance(manifest1[key], dict) and isinstance(manifest2[key], dict):
                merge_manifests_in_order(
                    manifest1[key],
                    manifest2[key],
                    manifest1_name,
                    manifest2_name,
                ...
                if ".".join(path) == "loader.env" and key in [
                    "LD_LIBRARY_PATH",
                    "PATH",
                    "LD_PRELOAD",
                ]:
                    ...
                    print(
                        f'Warning: Duplicate key `{".".join(path + [str(key)])}`. Concatenating'
                        f" values from `{manifest1_name}` and `{manifest2_name}`."
                    )
                else:
                    print(
                        f'Warning: Duplicate key `{".".join(path + [str(key)])}`. Overriding'
...
def handle_redhat_repo_configs(distro, tmp_build_path):
    ...
    with open("/etc/yum.repos.d/redhat.repo") as redhat_repo:
        ...
        if not re.search(repo_name, redhat_repo_contents):
            print(
                f"Cannot find {repo_name} in /etc/yum.repos.d/redhat.repo. "
...
def get_image_distro(docker_socket, image_name):
    out = docker_socket.containers.run(
        image_name, entrypoint="cat /etc/os-release", remove=True
    )
    out = out.decode("UTF-8")

    os_release = dict(
        shlex.split(line)[0].split("=") for line in out.splitlines() if line.strip()
    )
    ...
    if os_release["ID"] == "rhel":
        try:
            docker_socket.containers.run(
                image_name, entrypoint="ls /usr/bin/microdnf", remove=True
            )
...
def gsc_build(args):
    ...
    unsigned_image_name = gsc_unsigned_image_name(
        args.image
    )  # output unsigned image name
    ...
    if "Image" in config["Gramine"]:
        ...
        if get_docker_image(docker_socket, gramine_image_name) is None:
            ...
            if (
                get_docker_image(docker_socket, gsc_image_name(gramine_image_name))
                is None
            ):
            ...
            print(
                f"Warning: The base-Gramine Docker image `{gramine_image_name}` was generated by"
                " an older version of GSC. Please re-build this image using this GSC version."
                " This warning will become an error in the future."
            )

    print(
        f"Building unsigned graminized Docker image `{unsigned_image_name}` from original "
    ...
    try:
        distro = fetch_and_validate_distro_support(
            docker_socket, original_image_name, env
        )
    ...
    try:
        ...
        print(
            f'Failed to parse the "{distro}/entrypoint.manifest.template" file. Error:',
            e,
            file=sys.stderr,
        )
        sys.exit(1)

    base_image_environment = extract_environment_from_image_config(
        original_image.attrs["Config"]
    )
    ...
    with open(user_manifest_name, "r") as user_manifest_file:
        ...
        try:
            ...
            print(
                f'Failed to parse the "{user_manifest_name}" file. Error:',
                e,
                file=sys.stderr,
            )
            sys.exit(1)

    merged_manifest_dict = merge_manifests_in_order(
        user_manifest_dict,
        entrypoint_manifest_dict,
        user_manifest_name,
        entrypoint_manifest_name,
    )
    merged_manifest_name = (
        f"<merged {user_manifest_name} and {entrypoint_manifest_name}>"
    )
    merged_manifest_dict = merge_manifests_in_order(
        merged_manifest_dict,
        base_image_env_dict,
        merged_manifest_name,
        base_image_env_name,
    )
    ...
    shutil.copyfile(
        os.path.join(KEYS_PATH, "intel-sgx-deb.key"),
        tmp_build_path / "intel-sgx-deb.key",
    )
    ...
    build_docker_image(
        ...
        buildargs=extract_build_args(args),
    )

    # Check if docker build failed
    if get_docker_image(docker_socket, unsigned_image_name) is None:
        print(
            f"Failed to build unsigned graminized Docker image `{unsigned_image_name}`."
        )
        sys.exit(1)

    print(
        f"Successfully built an unsigned graminized Docker image `{unsigned_image_name}` from "
...
def gsc_build_gramine(args):
    ...
    if distro == "auto":
        print(
            "`gsc build-gramine` does not allow `Distro` set to `auto` in the configuration "
            "file."
        )
        sys.exit(1)
    ...
    if not os.path.exists(distro_path):
        print(
            f"{distro} distro is not supported by GSC. (Template path: {distro_path})"
        )
        sys.exit(1)
    ...
    if args.file_only:
        print(
            f"Successfully created Dockerfile.compile for base-Gramine Docker image "
...
def gsc_sign_image(args):
    ...
    if unsigned_image is None:
        print(
            f"Cannot find unsigned graminized Docker image `{unsigned_image_name}`.\n"
            f"You must first build this image via `gsc build` command."
        )
        sys.exit(1)

    print(
        f"Signing graminized Docker image `{unsigned_image_name}` -> `{signed_image_name}`..."
    )
    ...
    try:
        ...
        build_docker_image(
            ...
            "Dockerfile.sign",
            forcerm=True,
            buildargs={"passphrase": args.passphrase, "BUILD_ID": build_id},
        )
    finally:
    ...
    if get_docker_image(docker_socket, signed_image_name) is None:
        print(
            f"Failed to build a signed graminized Docker image `{signed_image_name}`."
        )
        sys.exit(1)

    print(
        f"Successfully built a signed Docker image `{signed_image_name}` from "
...
def gsc_info_image(args):
    ...
    if gsc_image is None:
        print(
            f"Could not find graminized Docker image {args.image}.\n"
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        if not sigstruct:
            print(
                f"Could not extract Intel SGX-related information from image {args.image}."
            )
            sys.exit(1)
...
sub_build_gramine = subcommands.add_parser(
    "build-gramine", help="Build base-Gramine Docker image"
)
sub_build_gramine.set_defaults(command=gsc_build_gramine)
sub_build_gramine.add_argument(
    "-b",
    "--buildtype",
    choices=["release", "debug", "debugoptimized"],
    default="release",
...
sub_build_gramine.add_argument(
    "image", help="Name of the output base-Gramine Docker image."
)
...
sub_sign.add_argument(
    "key", help="Key to sign the Intel SGX enclaves inside the Docker image."
)
sub_sign.add_argument(
    "-p", "--passphrase", "--password", help="Passphrase for the signing key."
)
sub_sign.add_argument(
    "-D", "--define", action="append", default=[], help="Set image sign-time variables."
)
sub_sign.add_argument(
    "--remove-gramine-deps",
    action="append_const",
    dest="define",
    const="remove_gramine_deps=true",
...
sub_sign.add_argument(
    ...
    action="append_const",
    dest="define",
    const="remove_gramine_deps=false",
    help="Retain Gramine dependencies that are not needed" " at runtime.",
)
sub_info = subcommands.add_parser(
    "info-image", help="Retrieve information about a graminized " "Docker image"
)
