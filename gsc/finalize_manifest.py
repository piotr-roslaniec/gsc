...
def generate_trusted_files(root_dir, already_added_files):
    ...
    for dirpath, dirnames, files in os.walk(root_dir.encode("UTF-8"), followlinks=True):
        ...
        for file in files:
            ...
            if not is_utf8(filename):
                ...
                print(
                    f"\t[from inside Docker container] File {filename} is not in UTF-8!"
                )
                sys.exit(1)
            ...
            if not os.access(filename, os.R_OK):
                ...
                print(
                    f"\t[from inside Docker container] File {filename} is inaccessible!"
                )
                continue
    ...
    print(
        f"\t[from inside Docker container] Found {num_trusted} files in `{root_dir}`."
    )
...
def generate_library_paths():
    ...
    ld_paths = subprocess.check_output(
        "ldconfig -v -N -X", stderr=subprocess.PIPE, shell=True
    )
...
def main(args=None):
    ...
    if not os.path.isdir(args.dir):
        argparser.error(
            f"\t[from inside Docker container] Could not find directory `{args.dir}`."
        )

    env = jinja2.Environment(loader=jinja2.FileSystemLoader("/"))
    env.globals.update(
        {"library_paths": generate_library_paths(), "env_path": os.getenv("PATH")}
    )
    ...
    if "allow_all_but_log" not in rendered_manifest_dict["sgx"].get(
        "file_check_policy", ""
    ):
        trusted_files = generate_trusted_files(args.dir, already_added_files)
        rendered_manifest_dict["sgx"].setdefault("trusted_files", []).extend(
            trusted_files
        )
    else:
        print(
